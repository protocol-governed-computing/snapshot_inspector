...
Declares `si.artifact.refs` in full: what it admits, how it is presented, and which implementation
...
catalog:
  category: ARTIFACTS
  label: References
  summary: Consumers (incoming edges) and dependencies (outgoing edges), direct or transitive.
...
handler:
  ...
  operation: si.artifact.refs
  implementation:
    module: inspector.queries.artifact_refs
    callable: artifact_refs
  payload_template:
