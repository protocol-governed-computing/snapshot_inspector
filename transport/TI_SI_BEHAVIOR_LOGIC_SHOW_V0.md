# TI_SI_BEHAVIOR_LOGIC_SHOW_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.behavior_logic.show`

Declares `si.behavior_logic.show` in full: what it admits, how it is presented, and which implementation
...
catalog:
  category: BEHAVIOR
  label: Logic Graph
  summary: One workflow's compiled execution graph and its rendered projection.
...
handler:
  ...
  operation: si.behavior_logic.show
  implementation:
    module: inspector.reads.behavior_logic_show
    callable: behavior_logic_show
  payload_template:
    wf: "${input.wf}"
```
