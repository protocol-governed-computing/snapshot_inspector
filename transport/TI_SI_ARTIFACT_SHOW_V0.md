# TI_SI_ARTIFACT_SHOW_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.artifact.show`

Declares `si.artifact.show` in full: what it admits, how it is presented, and which implementation
...
```yaml
fqdn: inspection::TI_SI_ARTIFACT_SHOW_V0
...
catalog:
  category: ARTIFACTS
  label: Show
  summary: The published canonical artifact for an FQDN, as authored.
...
handler:
  ...
  operation: si.artifact.show
  implementation:
    module: inspector.reads.artifact_show
