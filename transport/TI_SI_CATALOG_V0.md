...
Declares `si.catalog` in full: what it admits, how it is presented, and which implementation
...
```yaml
fqdn: inspection::TI_SI_CATALOG_V0
...
catalog:
  category: SNAPSHOT
  label: Catalog
  summary: Every inspection operation this snapshot inspector answers.
...
handler:
  ...
  operation: si.catalog
  implementation:
    module: inspector.reads.catalog
    callable: catalog
  payload_template:
    {}
```
