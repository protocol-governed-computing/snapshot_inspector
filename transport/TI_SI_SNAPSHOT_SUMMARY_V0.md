# TI_SI_SNAPSHOT_SUMMARY_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.snapshot.summary`

Declares `si.snapshot.summary` in full: what it admits, how it is presented, and which implementation
...
```yaml
fqdn: inspection::TI_SI_SNAPSHOT_SUMMARY_V0
...
catalog:
  category: SNAPSHOT
  label: Summary
  summary: Composition identity, domains, and published counts by kind.
...
handler:
  ...
  operation: si.snapshot.summary
  implementation:
    module: inspector.reads.snapshot_summary
    callable: snapshot_summary
  payload_template:
    {}
```
