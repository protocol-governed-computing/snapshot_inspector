# TI_SI_VOCAB_RESOLVE_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.vocab.resolve`

Declares `si.vocab.resolve` in full: what it admits, how it is presented, and which implementation
...
catalog:
  category: VOCABULARY
  label: Resolve
  summary: Identity → per-domain addresses, or an address within one domain → identity.
...
handler:
  ...
  operation: si.vocab.resolve
  implementation:
    module: inspector.reads.vocab_resolve
    callable: vocab_resolve
  payload_template:
