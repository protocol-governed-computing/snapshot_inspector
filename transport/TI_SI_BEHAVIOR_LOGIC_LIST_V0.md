# TI_SI_BEHAVIOR_LOGIC_LIST_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.behavior_logic.list`

Declares `si.behavior_logic.list` in full: what it admits, how it is presented, and which implementation
...
```yaml
fqdn: inspection::TI_SI_BEHAVIOR_LOGIC_LIST_V0
...
catalog:
  category: BEHAVIOR
  label: Workflows
  summary: Every workflow carrying a published behavior-logic graph.
...
handler:
  ...
  operation: si.behavior_logic.list
  implementation:
    module: inspector.reads.behavior_logic_list
    callable: behavior_logic_list
  payload_template:
    {}
```
