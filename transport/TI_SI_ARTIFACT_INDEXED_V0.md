# TI_SI_ARTIFACT_INDEXED_V0

**Kind:** Transport Ingress Contract (Transport Standard V0 §6)
**Operation Identity:** `si.artifact.indexed`

Declares `si.artifact.indexed` in full: what it admits, how it is presented, and which implementation
...
catalog:
  category: ARTIFACTS
  label: Indexed
  summary: Whether an FQDN is carried by this composition's artifact index.
...
handler:
  ...
  operation: si.artifact.indexed
  implementation:
    module: inspector.reads.artifact_indexed
    callable: artifact_indexed
  payload_template:
